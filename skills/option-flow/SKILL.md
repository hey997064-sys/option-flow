...
- `_wan` → `万张`
- `strike` → 整数去小数点（`\$240`），非整数保留一位小数（`\$15.5`）；禁四舍五入归整
- `pcr_oi` → 直接 3 位小数（`0.791`）
- **美元号一律转义**：strike / 价格前的 `$` 在输出里写成 `\$`（如 `\$240` / `\$15.5`）。原因——带 LaTeX 数学渲染的 Markdown（Claude Code 聊天框、GitHub 等）会把一段里成对出现的 `$` 当行内公式定界符，吞掉中间的数字和 `**`，渲染成空的 `****`；`\$` 是转义的字面美元号，不再触发公式，且 markdown-it（report-renderer 所用）等渲染器会把 `\$` 还原成 `$`、不留反斜杠。⚠️ 例外：§3 ASCII 代码块内的 `$` 由 `compute.py` 生成、在代码块内本就不受公式渲染影响，**保持裸 `$`，不要转义**（代码块内 `\$` 反而会露出反斜杠）。
...
`iv_peak = None` 时：第一行改"无明显近期 IV 凸点（近端与远端 IV 接近）"，末句改为："IV 期限结构平稳，市场无近期事件溢价；" + regime bridge clause（基于 `read_states.iv_regime`：偏贵→"卖方收权利金占优"；偏便宜→"买方占优，做多波动率划算"；合理→"定价合理，方向比波动率更重要"）。约 20-30 字，不必扩写更多。
...
- `call_wall=null` 时：
  - §2 表 `Call/Put Wall` 行**同行两栏对位**：数值列 `— Wall 缺失 / \${put_wall.strike}`，距离列 `— / {put_wall.distance_pct:+.1f}%`
...
- `put_wall=null` 时：对称处理 put 侧——
  - §2 表数值列 `\${call_wall.strike} / — Wall 缺失`，距离列 `{call_wall.distance_pct:+.1f}% / —`
...
→ IV 期限结构平稳，市场无近期事件溢价；定价合理，方向比波动率更重要。
```
